  ...
  return (
    <div style={{ background: PAPER, minHeight: "100vh", color: INK, fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "28px 24px 60px" }}>
        ...
        {view === "intervention" && (<div style={{background:PANEL,border:`1px solid ${LINE}`,padding:32}}><SectionLabel color={MOSS}>Intervention</SectionLabel><div style={{fontFamily:"Georgia, serif",fontSize:25,fontWeight:700,marginBottom:8}}>Prioritized action for known flood-prone areas</div><div style={{fontSize:13,color:MUTED,marginBottom:22}}>Use the Flood Intelligence diagnosis to validate and route practical interventions before the next rainfall event.</div>{LOCATIONS.map(l=><div key={l.id} style={{padding:"16px 0",borderTop:`1px solid ${LINE}`,display:"flex",justifyContent:"space-between",gap:18,flexWrap:"wrap"}}><div><b>{l.name}</b><div style={{fontSize:12,color:MUTED,marginTop:5}}>{l.options.find(o=>o.recommended)?.name}</div></div><span style={{fontSize:12,color:MOSS,fontWeight:700}}>Recommended first</span></div>)}</div>)}
